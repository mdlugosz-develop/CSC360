...
const { a } = require("natural/lib/natural/stemmers/indonesian/stemmer_id");
...
const Product = class Product {
    ...
    constructor(input){
        ...
        const propoerties = input.split(',')

        this.name = propoerties[0]
        this.price = Number(propoerties[1])
        this.availability = propoerties[2]
    }
     ...
     * Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array 
     */
    static halfOff = (products) => {return products.map(element => new Product(`${element.name},${element.price * .5},${element.availability}`) )}
    ...
    static printProducts = (products) => {
        products.forEach(element => {
            ...
            console.log(`Product: ${element.name}, Cost: ${new Intl.NumberFormat('en-US',{ style: 'currency', currency: 'USD'}).format(element.price)}, Availability: ${available}`)
